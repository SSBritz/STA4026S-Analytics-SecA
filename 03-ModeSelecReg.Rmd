...
Starting with either the null (forward stepwise) or saturated (backward stepwise) model, predictors are sequentially added or removed respectively according to some improvement metric. Once can also apply a hybrid method, which considers both adding and removing a variable at each step.
...
Of course, now there are two parameters to tune simultaneously, and the choice of $\alpha$ influences the range of $\lambda$ value we should consider searching over -- compare the x-axis ranges of Figures \@ref(fig:ridge-cv) and \@ref(fig:lasso-cv).

At time of writing, to the author's knowledge, there is no R package on CRAN that allows one to automatically search over both tuning parameters, or **hyperparameters** and find the optimal combination. Therefore, there are three options:
...
1. Apply elastic-net regression to the prostate cancer dataset using one of the suggested methods in section \@ref(elastic-net).
