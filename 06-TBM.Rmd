...
In the code below we see that one most specify the pruning cost complexity function, which we will set to the misclassification rate. 
...
titanic_bigtree <- tree(Survived ~ ., data = titanic, 
                        control = tree.control(nobs = nrow(na.omit(titanic)), mindev = 0.005))
...
Although these boundaries capture the underlying pattern seemingly well, we might well ask whether non-orthogonal boundaries would not have been more appropriate -- either linear or non-linear. That raises the question of which approach/model is "best". 
