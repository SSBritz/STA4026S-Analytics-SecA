...
```{r b-v-trade, warning=FALSE, fig.dim=c(6,6), fig.align='center', fig.cap='I must still write this figure caption!!'}
set.seed(1)
...
With this approach, the training set is randomly divided in to $k$ groups, or folds, of (approximately) the same size. Each fold gets a turn to act as the validation set, with the model trained on the remaining $k-1$ folds. Therefore, the training process is repeated $k$ times, each yielding an estimate of the test error, denoted as $MSE_1,\ MSE_2,\ldots,\ MSE_k$. These values are then averaged over all $k$ folds to yield the **$k$-fold CV estimate**:
...
Also note that as $k$ increases, the computational cost increase proportionally, since $k$ separate models must be fitted to $k$ different data splits. This could cause unnecessarily long training times for large datasets/complicated models, such that a smaller $k$ might be preferable. 
...
```{r, animation.hook='gifski', interval=1, out.width='150%'}
set.seed(4026)
...
for(k in 1:10){
  ...
  legend('bottomright', c('Training', 'Validation', 'Errors'), 
         ...
         lty = c(NA, NA, 3),
         lwd = c(NA, NA, 2))
  plot(1:k, cv_k, 'b', col = 'red', lwd = 2, xlab = 'Fold', ylab = 'MSE', 
...
```{r}
#dofs <- 5:20 #Don't need to go overboard, the min will be in this range
...
for(k in 1:10){
  ...
  for(dof in dofs){ #Using the same dofs as earlier
    d <- d + 1
    fit <- smooth.spline(x[-valid_ind], y[-valid_ind], df = dof)
    test <- predict(fit, x[valid_ind])$y
    fold_mses[k, d] <- mean((test - y[valid_ind])^2)
  }
}

valid_mses <- colMeans(fold_mses)

# Add the true MSE from earlier
plot(dofs, results$MSE, 'l', col = 'darkred', lwd = 2,
     xlab = 'Model complexity', ylab = '', ylim = c(0, max(valid_mses)))
lines(dofs, valid_mses, 'l', lwd = 2)
legend('bottomright', c('CV MSE', 'True test MSE'), col = c('black', 'darkred'), lty = 1, lwd = 2)
abline(v = dofs[which.min(results$MSE)], lty = 3)
points(dofs[which.min(valid_mses)], min(valid_mses), pch = 13, cex = 2.5, lwd = 2)
```
...
1. For Example 1, edit the provided code such that LOOCV is applied. Does this method suggest a different model complexity? What about 5-fold CV?
